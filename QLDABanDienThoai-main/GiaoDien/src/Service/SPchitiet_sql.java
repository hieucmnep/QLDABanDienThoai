    ...
    public void  hideRowInDatabase(String ma) {
        ...
        try {
            String sql =   "UPDATE SANPHAMCT SET hidden = 0 WHERE MASP = ? \n"
                            +"UPDATE SANPHAM SET hidden = 0 ,TRANGTHAI = 1 WHERE MASP = ?";                         
    ...
    public void  showInDatabase(String ma) {
        ...
        try {
            String sql =   "UPDATE SANPHAMCT SET hidden = 1 WHERE MASP = ? \n"
                            +"UPDATE SANPHAM SET hidden = 1 ,TRANGTHAI = 1"
                            + " WHERE MASP = ?";                         
            PreparedStatement stm = cn.prepareStatement(sql);
            stm.setString(1, ma);
            stm.setString(2, ma);
...
//            }
    ...
    public int ThemSPCT(SPChiTiet sp) {
        ...
        try {
            Connection cn = DBConnector.getConnection();
            String sql = "INSERT INTO SANPHAMCT (MASPCT,MAMAU, MASP, MARAM, MANSX,NGAYNHAP, SOLUONG)\n"
                        + "VALUES(?,?,?,?,?,?,?)";
            stm = cn.prepareStatement(sql);
            ...
            stm.setString(6, sp.getNgaynhap());
